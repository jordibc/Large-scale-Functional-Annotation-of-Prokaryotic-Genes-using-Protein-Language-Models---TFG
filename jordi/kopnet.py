...
def main():
    ...
    with open(f"{metric_name}acc_per_class.txt", "w") as f:
        ...
        # Global Recall:
        recall = (0 if total_samples <= 0 else
                  sum(r * nt for r, nt in zip(per_class_recall, ntotal)) / total_samples)

        # Precision:
        precision = (0 if not per_class_precision else
                     sum(per_class_precision) / len(per_class_precision))

	# Weighted precision:
        weighted_precision = (0 if total_predictions <= 0 else
                              sum(p * pt for p, pt in zip(per_class_precision, predicted_totals)) / total_predictions)
